...
over the DT's life cycle [@talasila_composable_2025]. The corpus currently
...
accurate way to compose DTs" [@talasila_composable_2025]. The paper also
distinguishes two
...
composed [@talasila_composable_2025].

## Composition inside a single applied architecture (thin coverage)

[@noauthor_digital_nodate] does not address composition as a research question --
...
distinct composition formalism of its own [@noauthor_digital_nodate]. Coverage of
...
| Eclipse BaSyX (AAS) | [@talasila_composable_2025] (secondary; not a separate item in this library) | Aggregation of Sub-models | DT-DT relationships only implicit, via semantic identifiers/References |
| Eclipse Ditto | [@talasila_composable_2025] (secondary; not a separate item in this library) | Aggregation of Features | DT-DT relationships: N/A |
| DTDL (Microsoft Azure) | [@talasila_composable_2025] (secondary; not a separate item in this library) | Composition of Interfaces via Components; explicit DT-DT `Relationships` field | Requires a back-end to realize bidirectional sync (classified as Digital Model by default) |
| Eclipse Vorto | [@talasila_composable_2025] (secondary; not a separate item in this library) | Aggregation of Function Blocks | DT-DT relationships only implicit, via Model References |
| INTO-CPS co-simulation framework | [@talasila_composable_2025] (secondary; not a separate item in this library) | Aggregation of hierarchical FMU simulators | DT-DT relationships: N/A |
| Applied DTaaS-aligned architecture for a mobile robot | `noauthor_digital_nodate` | Composes Data/Operations/State/Behaviors/Monitors around one robot via MQTT; self-identifies as a DTaaS instance | Does not address reuse, configuration validity, or DT-to-DT composition; single-instance demonstration, not a general mechanism [@noauthor_digital_nodate] |
...
(`talasila_composable_2025`) treats composition as a first-class research
...
- **Formal/semantic verification of composed DTs.** `talasila_composable_2025`
...
- **Standardized composition/configuration languages.** DTDL, AAS, and
  ...
  claim about them beyond what `talasila_composable_2025` reports should be
  attributed to them.
...
- **Runtime-verification-driven composition patterns generalized beyond a
  single robot.** `noauthor_digital_nodate`'s Monitors/Behaviors/Data/State
