...
\texttt{read\_sensor()} is what a real capacitive soil-moisture probe
gives you -- the true value, plus a little sensor noise.
\texttt{random.seed(7)} pins that randomness so your output matches this
tutorial's exactly.
...
\texttt{read\_sensor()}, the same way a real twin only sees whatever its
sensor's network connection delivers. Append this to \texttt{twin.py}:
...
recent trend matters for what comes next. \texttt{predict\_next()} is
the twin's model -- deliberately the simplest one that works: extend the
line between the last two readings one more hour forward.
\texttt{decide()} is the twin's algorithm: if that predicted value would
be too dry, it's time to water. This is exactly the three-part shape
every digital twin shares -- data in, a model to interpret it, an
algorithm to act on it -- just small enough here to read in one sitting.
...
Save the file and run \texttt{python3\ twin.py} again. Watering now
happens at hour 8 instead of hour 12, and a second time at hour 21:
...
replace \texttt{predict\_next} with a Kalman filter, which blends the
model's prediction against each new noisy reading instead of trusting
the latest one alone {[}1{]}, and would read real sensor data over MQTT
rather than calling a Python method directly. A how-to guide would be
the right place to cover wiring up an actual moisture probe and pump,
since that involves choices -- which board, which protocol -- that a
first tutorial deliberately avoids.

\subsection{Further reading}\label{further-reading}

{[}1{]} H. Feng, C. Gomes, and P. G. Larsen, \emph{Model-Based
Monitoring and State Estimation for Digital Twins: The Kalman Filter},
arXiv, 2023. \texttt{feng\_model-based\_2023}
